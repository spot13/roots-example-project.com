	...
	function green_options_is_used() {
		...
		if (is_admin()) {
			if (isset($_REQUEST['action']) && ($_REQUEST['action']=='green_options_save' || $_REQUEST['action']=='green_options_import'))		// AJAX: Save or Import Theme Options
				$used = true;
			else if (green_strpos($_SERVER['REQUEST_URI'], 'green_options')!==false)															// Edit Theme Options
				$used = true;
			else if (green_strpos($_SERVER['REQUEST_URI'], 'post-new.php')!==false || green_strpos($_SERVER['REQUEST_URI'], 'post.php')!==false) {	// Create or Edit Post (page, product, ...)
				$post_type = green_admin_get_current_post_type();
				if (empty($post_type)) $post_type = 'post';
				$used = green_get_override_key($post_type, 'post_type')!='';
			} else if (green_strpos($_SERVER['REQUEST_URI'], 'edit-tags.php')!==false) {															// Edit Taxonomy
				$inheritance = green_get_theme_inheritance();
				if (!empty($inheritance)) {
					$post_type = green_admin_get_current_post_type();
					if (empty($post_type)) $post_type = 'post';
					foreach ($inheritance as $k=>$v) {
						if (!empty($v['taxonomy'])) {
							foreach ($v['taxonomy'] as $tax) {
								if ( green_strpos($_SERVER['REQUEST_URI'], 'taxonomy='.($tax))!==false && in_array($post_type, $v['post_type']) ) {
									$used = true;
									break;
								}
							}
	...
	function green_options_menu_list($field, $clone_val) {
		...
		foreach ($field['options'] as $key => $item) {
			...
			if (in_array($field['type'], array('list', 'select', 'fonts')))
				...
				//$list .= '<img src="'.esc_attr($item).'" data-icon="'.esc_attr($key).'" alt="" class="green_options_input_image" />';
		...
		foreach ($field['val'] as $clone_num => $clone_val) {
			...
			case 'media':
				...
				if (!empty($clone_val) && !green_is_inherit_option($clone_val)) {
					...
					echo '<a class="green_options_image_preview" data-rel="popup" target="_blank" href="'.esc_url($clone_val).'">'.(!empty($ext) && green_strpos('jpg,png,gif', $ext)!==false ? '<img src="'.esc_url($clone_val).'" alt="" />' : '<span>'.($info['basename']).'</span>').'</a>';
				}
			break;
