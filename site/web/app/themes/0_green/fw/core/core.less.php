...
// Theme init
if (!function_exists('green_less_theme_setup')) {
	//add_action( 'green_action_before_init_theme', 'green_less_theme_setup' );
	function green_less_theme_setup() {
		// Recompile LESS and save CSS
		add_filter("green_filter_save_options", 'green_less_save_stylesheet', 10, 3);
	}
}

if (!function_exists('green_less_theme_setup2')) {
	//add_action( 'green_action_after_init_theme', 'green_less_theme_setup2' );
	function green_less_theme_setup2() {
		// Theme first run - compile and save css
		$theme_data = wp_get_theme();
		$slug = str_replace(' ', '_', trim(green_strtolower((string) $theme_data->get('Name'))));
		$option_name = 'green_'.strip_tags($slug).'_less_compiled';
		if ( get_option($option_name, false) === false ) {
			add_option($option_name, 1, '', 'yes');
			green_less_save_stylesheet(green_options_get_all_values(), 'general');
		} else if ( !is_admin() && green_get_theme_option('debug_mode')=='yes' ) {
			//green_less_save_stylesheet(green_options_get_all_values(), 'general');
		}
	}
}
...
if ( !function_exists( 'green_less_save_stylesheet' ) ) {
	//add_filter("green_filter_save_options", 'green_less_save_stylesheet', 10, 3);
	function green_less_save_stylesheet($options, $override, $slug) {
		if ($override == 'general') {
			// Compile CSS from LESS files
			green_compile_less($options);
		}
		return $options;
	}
}
...
if (!function_exists('green_compile_less')) {	
	function green_compile_less($options) {
...
if (!function_exists('green_prepare_less')) {	
	function green_prepare_less($options) {
		$vars = $styles = '';
		/*
		$fonts_list = array( 
			'content', 
...
/*
if (!function_exists('green_save_css')) {	
	function green_save_css($styles, $filename = '') {
