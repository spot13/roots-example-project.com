<?php
//===================================== Post author info =====================================
if (green_get_custom_option("show_post_author") == 'yes') {
	$post_author_name = $post_author_socials = '';
	$show_post_author_socials = false;
	if ($post_data['post_type']=='post') {
		$post_author_title = esc_html__('About', 'green');
		...
		$post_author_avatar = get_avatar($post_author_email, 75*min(2, max(1, green_get_theme_option("retina_ready"))));
		$post_author_descr = do_shortcode(nl2br(get_the_author_meta('description', $post_data['post_author_id'])));
		if ($show_post_author_socials) $post_author_socials = green_show_user_socials(array('author_id'=>$post_data['post_author_id'], 'style'=>'bg', 'size'=>'tiny', 'echo' => false));
	} 
	if (!empty($post_author_name)) {
		...
		<section class="post_author author vcard" itemprop="author" itemscope itemtype="http://schema.org/Person">
			...
			<h6 class="post_author_title">About author <span itemprop="name"><a href="<?php echo esc_url($post_author_url); ?>" class="fn"><?php echo ($post_author_name); ?></a></span></h6>
